...
namespace Util.ApplicationServices {
    ...
    public class TenantUploadPathStrategy : IUploadPathStrategy {
        ...
        private string GetTenantCategory() {
            if ( string.IsNullOrWhiteSpace( SecurityContext.Identity.TenantCode ) )
                return "Default";
            return SecurityContext.Identity.TenantCode;
        }
        ...
        private string GetUserId() {
            if ( string.IsNullOrWhiteSpace( SecurityContext.Identity.UserId ) )
                return "Default";
            return SecurityContext.Identity.UserId;
        }
    }
}
